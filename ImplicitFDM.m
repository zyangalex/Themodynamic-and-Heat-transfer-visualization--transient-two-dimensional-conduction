...
% Intialize all solution arrays
Tsolve = T1*ones((Nx+1)*(Ny+1),1); % Initialize T array to T1 everywhere
T = zeros(Ny+1,Nx+1);
...
for L = 1:Lmax          % Loop through time steps
    Told = Tsolve;           % Store previous T array as Told for next time step
    %Initialize matrices needed for solution
    C = zeros((Nx+1)*(Ny+1),1);
    A = zeros((Nx+1)*(Ny+1));
    for j = 1:Ny+1        % Loop through rows
        for i = 1:Nx+1    % Loop through columns
            
            % Calculates temperatures for new time step
            
            %Decide if you have a fixed temperature
            if (i == 1) || (i == Nx+1) || (j ==1)
            ...
            elseif (j==Ny+1) 
                ...
                A(node_id(j,i),node_id(j,i)) = (1+4*Fo);
                A(node_id(j,i),node_id(j+1,i)) = -Fo;
                A(node_id(j,i),node_id(j-1,i)) = -Fo;
                A(node_id(j,i),node_id(j,i+1)) = -Fo;
                A(node_id(j,i),node_id(j,i-1)) = -Fo;
                C(node_id(j,i)) = Told(node_id(j,i));
